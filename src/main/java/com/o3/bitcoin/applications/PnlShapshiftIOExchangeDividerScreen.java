 ...
 */
package com.o3.bitcoin.applications;
...
public class PnlShapshiftIOExchangeDividerScreen extends javax.swing.JPanel implements BasicScreen{
    ...
    private static final Logger logger = LoggerFactory.getLogger(PnlApplicationsScreen.class);
    ...
    private final DefaultComboBoxModel<String> coinsModel = new DefaultComboBoxModel<>(); 
    ...
    private List<String> coinNames = new ArrayList<String>();
    private List<String> coinSymbol = new ArrayList<String>();
    private List<String> coinIconURL = new ArrayList<String>();
    private List<ImageIcon> coinIcon = new ArrayList<ImageIcon>();
    ...
    private void getAvailableCoinsList() {
        new Thread(new Runnable() {
            @Override
            public void run() {
                try {
                    String result = HttpGetClient.getValuesFromUrl(ResourcesProvider.LTC_BTC_URLs.get("GetCoins"));
                    ...
                    while (keys.hasNext()) {
                        ...
                        if (jObject.get(key) instanceof JSONObject) {
                            ...
                            if(jO.getString("status").equalsIgnoreCase("available")) {
                                if(jO.getString("symbol").equalsIgnoreCase("BTC"))
                                    continue;
                                try {
                                    coinIcon.add(new ImageIcon(new java.net.URL(jO.getString("imageSmall"))));
                                    coinNames.add(jO.getString("name"));
                                    coinSymbol.add(jO.getString("symbol"));
                                    coinIconURL.add(jO.getString("imageSmall"));
                                }catch(Exception e) {
                                    System.out.println("coinIcon exception="+e.getMessage());
                                }
                    ...
                    System.out.println("Coins List fetch exception="+e.getMessage());
                    logger.error("Bitrefill coins list fetch exception = " + e.getMessage());
                }
                coinsModel.removeAllElements();
                for (String cn : coinNames) {
                    coinsModel.addElement(cn);
                }
         }
            }).start();       
    }
        ...
        if (marketInfoTimer == null) {
            marketInfoTimer = new Timer(30000, new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                    ...
                    depositCurrencyShortCode = coinSymbol.get(coinNames.indexOf(selectedCurrecy));
                    getMarketInfo();
                }
            });
    ...
    private void customizeUI() {
        ...
        cmbDeposit.setRenderer(new BasicComboBoxRenderer() {
            ...
            public Component getListCellRendererComponent(JList list, Object value, int index, boolean isSelected, boolean cellHasFocus) {
                ...
                component.setIcon(coinIcon.get(coinNames.indexOf(value.toString())));
                if (isSelected) {
    ...
    private void initComponents() {
        ...
        cmbDeposit.setModel(coinsModel);
