/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.o3.bitcoin.ui.dialogs;

import com.o3.bitcoin.Application;
import com.o3.bitcoin.exception.ClientRuntimeException;
import com.o3.bitcoin.ui.ApplicationUI;
import com.o3.bitcoin.ui.component.XButtonFactory;
import com.o3.bitcoin.ui.dialogs.screens.PnlLoginScreen;
import com.o3.bitcoin.util.ResourcesProvider;
import java.awt.Color;
import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;
import java.util.ArrayList;
import java.util.List;
import javax.swing.JButton;
import javax.swing.JOptionPane;
import javax.swing.JPanel;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 *
 * @author
 */

/**
 * <p>Class that implements ui dialog for login</p>
*/
public class DlgLogin extends BasicDialog {
    
    private static final Logger logger = LoggerFactory.getLogger(DlgLogin.class);
    private PnlLoginScreen pnlLoginScreen;
    private List<JButton> controls = new ArrayList<>();
    private boolean noWallet;
    /**
     * Creates new form DlgLogin
     *
     */
    public DlgLogin(boolean noWallet) {
        super(false);
        this.noWallet = noWallet;
        setupUI();
    }
    
    @Override
    protected JPanel getMainContentPanel() {
        if (pnlLoginScreen == null) {
            pnlLoginScreen = new PnlLoginScreen();
        }
        return pnlLoginScreen;
    }
    
    @Override
    protected List<JButton> getControls() {
        controls = super.getControls();
        controls.add(0, getLoginButton());
        return controls;
    }
    
    /**
     * function to get Login button for dialog and attach event handler
     * @return Login button
    */
    protected JButton getLoginButton() {
        JButton loginButton = new JButton("Login");
        XButtonFactory.themedButton(loginButton)
                .background(ResourcesProvider.Colors.NAV_MENU_WALLET_COLOR)
                .color(Color.WHITE)
                .font(ResourcesProvider.Fonts.BOLD_MEDIUM_FONT);
        
        loginButton.addActionListener(new ActionListener() {
            
            @Override
            public void actionPerformed(ActionEvent e) {
                handleLoginButtonClickEvent(e);
            }
        });
        return loginButton;
    }
    
    /**
     * callback function for Login button event 
    */
    protected void handleLoginButtonClickEvent(ActionEvent e) {
        try {
            boolean result = pnlLoginScreen.login();
            if (result) {
                Application.setLoggedIn(true);
                handleCloseDialogControlEvent(e);
            } else {
                throw new ClientRuntimeException("Incorrect Wallet Password.");
            }
        } catch (Exception ex) {
            logger.error("Login failed: {}", ex.getMessage(), ex);
            ApplicationUI.get().showError(ex);
        }
    }
    
    @Override
    protected String getHeadingText() {
        if( noWallet )
            return "Set Wallet Password";
        return "Login";
    }
    
    @Override
    protected void handleDefaultCloseEvent(ActionEvent e) {
        handleCloseDialogControlEvent(e);
    }

    /**
     * This method is called from within the constructor to initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is always
     * regenerated by the Form Editor.
     */
    @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        setDefaultCloseOperation(javax.swing.WindowConstants.DISPOSE_ON_CLOSE);

        javax.swing.GroupLayout layout = new javax.swing.GroupLayout(getContentPane());
        getContentPane().setLayout(layout);
        layout.setHorizontalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGap(0, 400, Short.MAX_VALUE)
        );
        layout.setVerticalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGap(0, 300, Short.MAX_VALUE)
        );

        pack();
    }// </editor-fold>//GEN-END:initComponents

    // Variables declaration - do not modify//GEN-BEGIN:variables
    // End of variables declaration//GEN-END:variables
}
